...
CY_ISR(ENC_ISR_Interrupt)
{
    ...
    /* `#START ENC_ISR_Interrupt` */
     CyDelayUs(50); 
            ...
            // Read current states   
            chA = CyPins_ReadPin(ENC_0); 
            //00 -> 10 -> 11 -> 01 -> 00
            //ENC = 0xBA   
  
            if(chA != lastChA){ // Check the state of A to see if it's changed direction. Use state  B to determine direction
                trigger++;
                if(trigger == 2){ // Divide State change
                    if(CyPins_ReadPin(ENC_1) != chA)
                    { 
                        enc_value += 1;
                    }//cw 
            ...
            lastChA = chA; // Set previous state to most current state 
...
void ENC_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)ENC_ISR__INTC_NUMBER] = address;
}
...
cyisraddress ENC_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)ENC_ISR__INTC_NUMBER];
}
